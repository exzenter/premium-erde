// ── Carousel (Grow Reports) ──
(function(){
  var current = 0;
  var total   = 2;
  var GAP     = 24;
  var track   = document.getElementById('nlh-carousel-track');
  var area    = document.getElementById('nlh-carousel-area');
  var dots    = document.getElementById('nlh-dots').querySelectorAll('button');
  ...
  function update() {
    ...
    if (isMobile) {
      ...
      Array.from(track.children).forEach(function(slide) {
        ...
        var grids = slide.querySelectorAll('[style*="grid-template-columns: 1fr 1fr 1fr"]');
        grids.forEach(function(g) { g.style.gridTemplateColumns = '1fr'; });
      });
    } else {
      slideW = Math.min(areaW * 0.78, 1200);
    }
    var offset = (areaW - slideW) / 2 - current * (slideW + GAP);
    track.style.transform = 'translateX(' + offset + 'px)';
    dots.forEach(function(d, i){ d.style.opacity = i === current ? '1' : '0.25'; });
  }

  window.nlhCarousel = function(dir){
    current = (current + dir + total) % total;
    update();
  };
  window.nlhGoTo = function(n){
    current = n;
    update();
  };

  window.nlhUpdate = update;
  update();
  window.addEventListener('resize', update);
})();

// ── Accordion (Grow Reports, mobile only) ──
(function(){
  var header = document.getElementById('nlh-accordion-header');
  var body   = document.getElementById('nlh-accordion-body');
  var chev   = document.getElementById('nlh-chevron');
  header.addEventListener('click', function(){
    if (window.innerWidth > 767) return;
    var open = body.classList.toggle('nlh-open');
    chev.classList.toggle('nlh-open', open);
    if (open && window.nlhUpdate) window.nlhUpdate();
  });
})();
...
function setUnit(unit) {
  const btnS = document.getElementById('btn-saecke');
  const btnP = document.getElementById('btn-paletten');
  const hint = document.getElementById('menge-hint');
  const hidden = document.getElementById('einheit-hidden');
  const input = document.getElementById('menge-input');
  if (unit === 'saecke') {
    btnS.style.background = '#1a3a0a'; btnS.style.color = '#fff';
    btnP.style.background = 'transparent'; btnP.style.color = '#888';
    hint.textContent = '40 L / Sack';
    hidden.value = 'Säcke';
    input.placeholder = 'Anzahl Säcke';
  } else {
    btnP.style.background = '#1a3a0a'; btnP.style.color = '#fff';
    btnS.style.background = 'transparent'; btnS.style.color = '#888';
    hint.textContent = '54 Säcke / Palette';
    hidden.value = 'Paletten';
    input.placeholder = 'Anzahl Paletten';
  }
}
window.setUnit = setUnit;

// ── LKW Accordion Toggle ──
window.toggleLkw = function() {
  var el = document.getElementById('lkw-accordion');
  var open = el.style.maxHeight && el.style.maxHeight !== '0px';
  el.style.maxHeight = open ? '0px' : el.scrollHeight + 'px';
  document.getElementById('lkw-toggle').textContent = open
    ? 'LKW-Lieferpreise anzeigen (ab 1 Palette möglich)'
    : 'LKW-Lieferpreise ausblenden';
};
...
document.getElementById('kontakt-form').addEventListener('submit', async function(e) {
  ...
  try {
    const res = await fetch('send.php', { method: 'POST', body: new FormData(this) });
    const data = await res.json();
    if (data.ok) {
      btn.textContent = 'Gesendet!';
      btn.style.background = '#90c96e';
      btn.style.color = '#1a3a0a';
      this.reset();
      setUnit('saecke');
    } else {
    ...
    btn.textContent = 'Fehler – direkt mailen: hanni@premium-erde.de';
...
// ── Leaf Reveal & Header Scroll ──
const LEAF_VB_W = 600;
const LEAF_VB_H = 640.187;

const header = document.getElementById("header");
const leafRevealArea = document.getElementById("leafRevealArea");
const overlay = document.getElementById("leafOverlay");

let lastScrollY = 0;

function applyLeafMask(progress) {
  ensureCanvas();
  resizeCanvas();
  ...
  const minScale = Math.min(w, h) * 0.1 / Math.max(LEAF_VB_W, LEAF_VB_H);
  const maxScale = Math.max(w, h) * 3.2 / Math.min(LEAF_VB_W, LEAF_VB_H);
  const easedProgress = Math.min(1, progress * progress / 0.7);
  const scale = minScale + (maxScale - minScale) * easedProgress;

  if (progress <= 0) {
    canvas.style.display = "block";
    drawLeafOnCanvas(0, cx, cy, minScale);
    return;
  }
  if (progress >= 1) {
    canvas.style.display = "block";
    drawLeafOnCanvas(1, cx, cy, maxScale);
    return;
  }

  canvas.style.display = "block";
  drawLeafOnCanvas(progress, cx, cy, scale);
}
...
function ensureCanvas() {
  ...
  canvas.style.cssText = "position:absolute;inset:-1px;width:calc(100% + 2px);height:calc(100% + 2px);z-index:3;pointer-events:none";
  overlay.parentElement.appendChild(canvas);
...
function drawLeafOnCanvas(progress, cx, cy, scale) {
  ensureCanvas();
  resizeCanvas();

  const w = canvas.width;
  const h = canvas.height;
  const leafCenterX = LEAF_VB_W / 2;
  const leafCenterY = LEAF_VB_H / 2;

  ctx2d.clearRect(0, 0, w, h);

  ctx2d.fillStyle = "#90c96e";
  ctx2d.fillRect(0, 0, w, h);

  ctx2d.save();
  ctx2d.globalCompositeOperation = "destination-out";

  ctx2d.translate(cx - leafCenterX * scale, cy - leafCenterY * scale);
  ctx2d.scale(scale, scale);

  const p2d = new Path2D(leafPathD);
  ctx2d.fill(p2d);
  ctx2d.restore();
  ...
  const lightGrad = ctx2d.createRadialGradient(cx, cy, 0, cx, cy, radius);
  lightGrad.addColorStop(0, "rgba(255, 255, 200, 0.3)");
  lightGrad.addColorStop(0.25, "rgba(255, 255, 180, 0.15)");
  lightGrad.addColorStop(0.5, "rgba(0, 0, 0, 0.1)");
  lightGrad.addColorStop(0.8, "rgba(0, 0, 0, 0.35)");
  lightGrad.addColorStop(1, "rgba(0, 0, 0, 0.55)");

  ctx2d.fillStyle = lightGrad;
  ctx2d.fillRect(0, 0, w, h);

  const edgeGrad = ctx2d.createRadialGradient(cx, cy, radius * 0.3, cx, cy, radius);
  edgeGrad.addColorStop(0, "rgba(0, 0, 0, 0)");
  edgeGrad.addColorStop(0.5, "rgba(0, 0, 0, 0)");
  edgeGrad.addColorStop(1, "rgba(0, 10, 0, 0.3)");

  ctx2d.fillStyle = edgeGrad;
  ctx2d.fillRect(0, 0, w, h);
